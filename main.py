#!/usr/bin/env python3
# main.py – rev-w37  (2025-06-27)
"""
Playlist-Player – gap-less playlist player with resume, cover-art,
timeline seek, Create/Rename/Delete lists, and Foobar .fplite support.

New in w37
──────────
• Uses **Playlist-Player_logo.ico** (in project root) as:
    – Qt application icon
    – Main-window icon
    – PyInstaller build icon (see build_windows.bat)
• All other functionality is unchanged from rev-w36.
"""

from __future__ import annotations
import sys, os, subprocess, venv, site, hashlib, io, re, json, tempfile
from pathlib import Path
from typing  import Dict, List, Optional, Tuple, Set

# ════════════════════════════════════════════════════════════════════════
# 0. Bootstrap: ensure local .venv with PySide6 / python-vlc / mutagen / Pillow
# ════════════════════════════════════════════════════════════════════════
...
# 2. Globals / constants
# ════════════════════════════════════════════════════════════════════════
ICON_PATH  = Path(__file__).with_name("Playlist-Player_logo.ico")   # ← NEW
...
TICKS       = 10        # 100 ms per slider step
MAX_SECONDS = 86_400    # 24 h clamp

# ════════════════════════════════════════════════════════════════════════
# 3. Helper utilities (strip_dpr  |  TimelineSlider)
# ════════════════════════════════════════════════════════════════════════
...
# ════════════════════════════════════════════════════════════════════════
# 4. CreatePlaylistDialog
# ════════════════════════════════════════════════════════════════════════
class CreatePlaylistDialog(QDialog):
    """Dialog to create a new playlist by selecting files & reordering."""
...
class MainWindow(QWidget):
    ...
    def __init__(self):
        ...
        if ICON_PATH.exists():
            self.setWindowIcon(QIcon(str(ICON_PATH)))       # ← NEW

        self.resize(1100,650)
        self.setAcceptDrops(True)
        self._init_style()

        # ── build UI (playlists list / tracks list / sidebar)
        self._build_widgets()
        self._wire_signals()
    ...
    def _build_widgets(self):
        ...
        split.addWidget(self.list_playlists); split.addWidget(self.tracks_sel); split.addWidget(sb)
        ...
        tb=QHBoxLayout(); [tb.addWidget(b) for b in(self.btn_create,self.btn_scan,self.btn_rename,self.btn_delete)]
        tb.addStretch(); tb.addWidget(self.btn_prev); tb.addWidget(self.btn_next)
    ...
    def _wire_signals(self):
        ...
        self.btn_prev.clicked.connect(self._player.prev_track); self.btn_next.clicked.connect(self._player.next_track)
    ...
    def _rename_playlist(self):
        ...
        if ok and new.strip():
            pl.name=new.strip(); self.list_playlists.currentItem().setText(pl.name); self._save_state()
    ...
    def _create_playlist(self):
        ...
        if self._add_playlists([pl]): self._save_state()
    ...
    def _tick(self):
        ...
        if self._player.player:
            length=max(1,min(MAX_SECONDS,self._player.length())); pos=max(0,min(self._player.position(),length))
    ...
    def closeEvent(self,e):
        self._player.flush_history()   # flush debounced history
        self._save_state()
        super().closeEvent(e)
...
if __name__=="__main__":
    ...
    if ICON_PATH.exists(): app.setWindowIcon(QIcon(str(ICON_PATH)))   # ← NEW
