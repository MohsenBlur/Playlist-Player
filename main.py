#!/usr/bin/env python3
# main.py – rev-w36  (2025-06-26)
"""
Playlist-Player - GUI front-end

• Scan folders for .m3u/.m3u8/.fplite (Foobar index.txt aware)
• Gap-less playback via libVLC
• Per-playlist resume history (debounced, atomic)
• Embedded cover-art, live progress bars, light/dark theme
• Create / Rename / Delete playlists
• Timeline seek (click/drag/wheel  ±5 s; Ctrl ±1 s)
"""

from __future__ import annotations
import sys, os, subprocess, venv, site, hashlib, io, re
...
TICKS       = 10        # timeline = 100 ms per tick
MAX_SECONDS = 86_400    # clamp length @ 24 h

def strip_dpr(px: QPixmap) -> QPixmap:
    """Normalize pixmap to DPR 1 for crisp scaling."""
...
class CreatePlaylistDialog(QDialog):
    ...
    def _move_sel(self,delta:int):
        ...
        if not rows: return
        new=[r+delta for r in rows]
        if min(new)<0 or max(new)>=self.list.count(): return
        items=[self.list.takeItem(r) for r in rows]
        for r,it in sorted(zip(new,items)): self.list.insertItem(r,it); self.list.setItemSelected(it,True)
...
class MainWindow(QWidget):
    ...
    def __init__(self):
        ...
        self.lbl_cover=QLabel(alignment=Qt.AlignCenter); self.lbl_cover.setFixedSize(self.ART_PX,self.ART_PX)
    ...
    def _cover(self,p:Path):
        ...
        return (strip_dpr(QPixmap(str(art)))
                .scaled(self.ART_PX,self.ART_PX,Qt.KeepAspectRatio,Qt.SmoothTransformation)
    ...
    # ------------------------------------------------------ create playlist (already defined earlier)

    # ------------------------------------------------------ close event
    def closeEvent(self,e):
        self._player.flush_history()       # ensure last snapshot persisted
        self._save_state()
        super().closeEvent(e)
