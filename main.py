#!/usr/bin/env python3
# main.py – rev-w34 (2025-06-25)
"""
Playlist-Relinker Player GUI

What’s new in w34
─────────────────
•  Playlist title now shown above the “Now Playing” list (right pane)  
•  “Prev” / “Next” buttons moved to the top-row toolbar, right-aligned  
•  Playback buttons use plain words (“Prev”, “Play/Pause”, “Next”)  
•  Mouse-wheel seeking on the timeline (± 5 s per notch, Ctrl = ± 1 s)  
•  All earlier features (gapless playback, progress bars, cover-art,
   drag-drop, etc.) retained.
"""
...
from PySide6.QtWidgets import (
    ...
    QLabel, QMessageBox, QFrame, QComboBox, QSlider
)
...
import scanner, storage, player, history   # local project modules
...
MAX_SECONDS = 86_400    # cap track length at 24 h

# ════════════════════════════════════════════════════════════════════════
#  helper utilities
# ════════════════════════════════════════════════════════════════════════
def strip_dpr(px: QPixmap) -> QPixmap:
    """Return DPR-1 pixmap so logical==physical pixels."""
...
class TimelineSlider(QSlider):
    """Clickable, draggable, scroll-wheel seek bar."""
    ...
    def mousePressEvent(self, e):
        if e.button() == Qt.LeftButton:
            ...
            v = self._val_from_x(int(e.position().x()
                          if hasattr(e, "position") else e.pos().x()))
    ...
    # NEW – wheel based seek
    def wheelEvent(self, e):
        step = 1 if e.modifiers() & Qt.ControlModifier else 5
        delta = step * (e.angleDelta().y() // 120)    # ±1 or ±5 s
        self.setValue(
            max(self.minimum(),
...
class MainWindow(QWidget):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Playlist-Relinker Player")
        ...
        # progress bars
        self._bar_sel  = QFrame(self.tracks_sel.viewport())
        self._bar_play = QFrame(self.tracks_cur.viewport())
        self._bar_sel .setStyleSheet("background:#00a29f;"); self._bar_sel .setFixedWidth(2); self._bar_sel .hide()
        self._bar_play.setStyleSheet("background:#00c8ff;"); self._bar_play.setFixedWidth(2); self._bar_play.hide()

        # sidebar (right column)
        self.cmb_output = QComboBox(); self.cmb_output.addItems(AUDIO_OPTIONS)
        self.lbl_cover  = QLabel(alignment=Qt.AlignCenter)
        self.lbl_cover.setFixedSize(self.ART_PX, self.ART_PX)
        self.lbl_cover.setStyleSheet("background:palette(Base);")
        ...
        # ── top toolbar buttons
        self.btn_scan, self.btn_rename, self.btn_delete = (
            QPushButton(t) for t in ("Scan", "Rename", "Delete")
        )
        self.btn_prev = QPushButton("Prev")
        self.btn_next = QPushButton("Next")

        top = QHBoxLayout()
        for b in (self.btn_scan, self.btn_rename, self.btn_delete):
            top.addWidget(b)
        top.addStretch()
        ...
        self.cmb_output.currentIndexChanged.connect(
            lambda i: self._player.set_output(AUDIO_MODES[i])
        )
    ...
    # ──────────────────────────────────────────────────────────
    #  drag & drop for adding playlists / folders
    # ──────────────────────────────────────────────────────────
    def dragEnterEvent(self, e: QDragEnterEvent):
        if any(Path(u.toLocalFile()).is_dir() or
               Path(u.toLocalFile()).suffix.lower() in scanner.PLAYLIST_EXTS
    ...
    def dropEvent(self, e: QDropEvent):
        ...
        for url in e.mimeData().urls():
            ...
            if p.is_dir():
                ...
                pl = next((pl for pl in
                           scanner.scan_playlists(p.parent, False)
                           if pl.path == p), None)
                if pl:
    ...
    # ──────────────────────────────────────────────────────────
    #  metadata & cover-art helpers
    # ──────────────────────────────────────────────────────────
    def _probe(self, data: bytes, mime: str, prio: int):
        """Return tuple used for selecting best art."""
        area = 0
        try:
    ...
    def _extract_art(self, audio, path: Path):
        ...
        if isinstance(audio.tags, ID3):
            for ap in audio.tags.getall("APIC"):
                cand.append(self._probe(ap.data,
                                        ap.mime or "image/jpeg",
                                        2 if ap.type == 3 else 1))
        if getattr(audio, "pictures", None):
            for pic in audio.pictures:
                cand.append(self._probe(pic.data,
                                        pic.mime or "image/jpeg",
                                        2 if getattr(pic, "type", 3) == 3 else 1))
        for stem in ("cover", "folder", "front", "AlbumArt", "Artwork"):
            for ext in (".jpg", ".jpeg", ".png"):
                f = path.parent / f"{stem}{ext}"
                if f.exists():
                    cand.append(self._probe(f.read_bytes(),
        ...
        try:
            ...
            if getattr(audio, "tags", None):
                title  = (audio.tags.get("TIT2") or
                          audio.tags.get("TITLE") or [""])[0]
                artist = (audio.tags.get("TPE1") or
                          audio.tags.get("ARTIST") or [""])[0]
    ...
    def _icon48(self, p):
        ...
        if art and art.exists():
            pix = strip_dpr(QPixmap(str(art))).scaled(
                48, 48, Qt.KeepAspectRatio, Qt.SmoothTransformation
            )
            self._icon_cache[p] = pix
            return QIcon(pix)
        return None

    def _cover(self, p: Path):
        _, _, art = self._meta(p)
        return (strip_dpr(QPixmap(str(art)))
                .scaled(self.ART_PX, self.ART_PX,
                        Qt.KeepAspectRatio, Qt.SmoothTransformation)
                if art and art.exists() else None)

    # ──────────────────────────────────────────────────────────
    #  persistence (playlist list)
    # ──────────────────────────────────────────────────────────
    ...
    def _save_state(self):
        storage.save([{"path": str(pl.path), "name": pl.name}
                      for pl in self._playlists])

    # ──────────────────────────────────────────────────────────
    #  playlist management (scan / add / rename / delete)
    # ──────────────────────────────────────────────────────────
    ...
    def _scan_folder(self):
        ...
        if folder and self._add_playlists(
                scanner.scan_playlists(Path(folder))):
            self._save_state()
    ...
    def _rename_playlist(self):
        ...
        new, ok = QInputDialog.getText(
            self, "Rename playlist", "Display name:", text=pl.name)
        if ok and new.strip():
            pl.name = new.strip()
    ...
    def _place_bar(self, bar: QFrame,
                   lw: QListWidget, idx: int, frac: float):
        """Draw a 2-px progress bar inside list-widget row."""
    ...
    # ──────────────────────────────────────────────────────────
    #  refresh “Now Playing” (right) pane
    # ──────────────────────────────────────────────────────────
    ...
    def _place_play_bar(self):
        ...
        self._place_bar(self._bar_play, self.tracks_cur,
                        self._player.idx, frac)
    ...
    def _tick(self):
        ...
        if self._player.player:
            ...
            # update timeline (avoid while dragging)
            ...
            # live update teal-bar in selection pane if same playlist
            sel_pl = self._sel_pl()
    ...
    def _on_track_change(self, *_):
        self.slider.setEnabled(True)
        self._cur_pl_idx = next(
            (i for i, pl in enumerate(self._playlists)
             if pl.path == self._player._pl_path), None
        )
        current_pl = self._sel_pl()
        self.lbl_curtitle.setText(
            current_pl.name if current_pl else "Now Playing")
        self._highlight_row()
        self._refresh_cur()
        self._refresh_sel()
        self.lbl_cover.setPixmap(
            self._cover(self._player.playlist[self._player.idx]) or QPixmap()
        )
