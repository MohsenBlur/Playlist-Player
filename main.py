#!/usr/bin/env python3
# main.py – rev-w39  (2025-06-27)
"""
Playlist-Player – gap-less music-player with Foobar .fplite support,
per-playlist resume history, cover-art, and timeline seek.

• Create / Scan / Rename / Delete playlists
• Gap-less playback (python-vlc) with WASAPI / DirectSound selector
• Debounced history writes + final flush on exit
• Custom icon  ▸  Playlist-Player_logo.ico  (project root)

This is the **FULL source**: every helper method is included.
"""
...
# ════════════════════════════════════════════════════════════════════════
# 0. Bootstrap: local .venv (PySide6 · python-vlc · mutagen · Pillow)
# ════════════════════════════════════════════════════════════════════════
...
TICKS       = 10        # 100 ms per slider tick
...
class MainWindow(QWidget):
    ...
    def __init__(self):
        ...
        self._load_state()                              # needs list widgets ready
    ...
    def _create_playlist(self):
        ...
        tracks=dlg.tracks();  ​if not tracks: return
        fname,_=QFileDialog.getSaveFileName(self,"Save playlist",str(Path.home()/"playlist.m3u8"),"M3U8 playlist (*.m3u8)")
        if not fname: return
        path=Path(fname)
    ...
    def _make_item(self,p:Path,prefix:str=""):
        it=QListWidgetItem(prefix+self._display(p)); ico=self._icon48(p)
        if ico: it.setIcon(ico)
        return it
    ...
    def _refresh_sel(self):
        ...
        hist=history.load(pl.path); finished=set(hist.get("finished",[]))
